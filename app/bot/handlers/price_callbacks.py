...
async def accept_price_callback(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        try:
            ...
            if success:
                # Сохраняем данные пользователя до закрытия сессии
                user_data = {
                ...
                await callback.message.edit_text(
                    f"✅ <b>Вы приняли цену {order.price} ₽</b>\n\n"
                    f"📋 <b>Заказ #{order.id}</b>\n"
                    f"📝 {order.work_type}: {order.topic[:50]}...\n\n"
        ...
        # Уведомляем админа о принятии цены (после закрытия сессии)
        if success:
            try:
    ...
    try:
        ...
        try:
            ...
            if success:
                ...
                await callback.message.edit_text(
                    f"❌ <b>Вы отклонили цену {order.price} ₽</b>\n\n"
                    f"📋 <b>Заказ #{order.id}</b>\n"
                    f"📝 {order.work_type}: {order.topic[:50]}...\n\n"
        ...
        # Уведомляем админа об отклонении цены (после закрытия сессии)
        if success:
            try:
                from app.bot.bot import bot
        ...
        try:
            ...
            if success:
                ...
                try:
                    ...
                    admin_text += f"📝 <b>Работа:</b> {order_data['work_type']}\n"
                    admin_text += f"📋 <b>Тема:</b> {order_data['topic'][:100]}...\n\n"
                    admin_text += f"🔗 <b>Админ-панель:</b> http://127.0.0.1:8000/orders/{order_id}\n\n"
                    admin_text += f"⏰ Статус изменен на: <b>Ожидает оплаты</b>"
